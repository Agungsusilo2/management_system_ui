...
export default function KelompokMk() {
    ...
    const fetchData = async () => {
        const response = await kelompokMkAll({ token: authToken, page, size });
        const result = await response.json();

        if (response.ok) {
            const formatted = result.data.map((item) => ({
                id: item.idKelompokMk,
                nama: item.namaKelompokMk,
            }));
            setData(formatted);
    ...
    const handleSubmit = async (formData) => {
        let res;
        if (selectedRow) {
            res = await kelompokMkUpdate({
                token: authToken,
                idKelompokMk: selectedRow.id,
                namaKelompokMk: formData.nama,
            });
        } else {
            res = await kelompokMkAdd({
                token: authToken,
                namaKelompokMk: formData.nama,
            });
        }
    ...
    const handleDelete = async (row) => {
        ...
        const res = await kelompokMkDelete({
            token: authToken,
            idKelompokMk: row.id,
        });
    ...
    const handleAIResponse = async (result) => {
        ...
        for (const item of result.items) {
            const payload = {
                token: authToken,
                namaKelompokMk: item.namaKelompokMk,
            };

            try {
                const res = await kelompokMkAdd(payload);
                const body = await res.json();

                if (!res.ok) {
                    await alertFailed(`❌ Gagal menambahkan: ${item.namaKelompokMk}`);
                }
            } catch (err) {
                console.error(err);
            }
    ...
    const columns = [
        { key: "id", label: "ID Kelompok MK" },
        { key: "nama", label: "Nama Kelompok MK" },
    ];
    ...
    return (
        <>
            {isAdmin && (
                <ChatAssistant
                    fields={["namaKelompokMk"]}
                    onAIResponse={handleAIResponse}
                />
            )}

            <DataTable
                title="Daftar Kelompok Mata Kuliah"
                data={data}
                columns={columns}
                onAdd={isAdmin ? handleAdd : undefined}
                onEdit={isAdmin ? handleEdit : undefined}
                onDelete={isAdmin ? handleDelete : undefined}
                searchPlaceholder="Cari kelompok MK..."
                pagination={{
                    page,
                    size,
                    total: totalItems,
