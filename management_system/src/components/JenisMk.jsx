...
export default function JenisMk() {
    ...
    const fetchData = async () => {
        const response = await jenisMkAll({ token: authToken, page, size });
        const result = await response.json();

        if (response.ok) {
            const formatted = result.data.map((item) => ({
                id: item.idJenisMk,
                nama: item.namaJenisMk,
            }));
            setData(formatted);
    ...
    const handleSubmit = async (formData) => {
        let res;
        if (selectedRow) {
            res = await jenisMkUpdate({
                token: authToken,
                idJenisMk: selectedRow.id,
                namaJenisMk: formData.nama,
            });
        } else {
            res = await jenisMkAdd({
                token: authToken,
                namaJenisMk: formData.nama,
            });
        }
    ...
    const handleAIResponse = async (result) => {
        ...
        for (const item of result.items) {
            const payload = {
                token: authToken,
                namaJenisMk: item.namaJenisMk,
            };

            try {
                const res = await jenisMkAdd(payload);
                const body = await res.json();

                if (!res.ok) {
                    await alertFailed(`❌ Gagal menambahkan: ${item.namaJenisMk}`);
                }
            } catch (err) {
                console.error(err);
            }
    ...
    const columns = [
        { key: "id", label: "ID Jenis MK" },
        { key: "nama", label: "Nama Jenis MK" },
    ];
    ...
    return (
        <>
            ...
            <DataTable
                title="Daftar Jenis Mata Kuliah"
                data={data}
                columns={columns}
                onAdd={isAdmin ? handleAdd : undefined}
                onEdit={isAdmin ? handleEdit : undefined}
                onDelete={isAdmin ? handleDelete : undefined}
                searchPlaceholder="Cari jenis MK..."
                pagination={{
                    page,
                    size,
                    total: totalItems,
