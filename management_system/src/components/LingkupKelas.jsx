...
export default function LingkupKelas() {
    ...
    const fetchData = async () => {
        const response = await lingkupKelasAll({ token: authToken, page, size });
        const result = await response.json();

        if (response.ok) {
            const formatted = result.data.map((item) => ({
                id: item.idLingkupKelas,
                nama: item.namaLingkupKelas,
            }));
            setData(formatted);
    ...
    const handleSubmit = async (formData) => {
        let res;
        if (selectedRow) {
            res = await lingkupKelasUpdate({
                token: authToken,
                idLingkupKelas: selectedRow.id,
                namaLingkupKelas: formData.nama,
            });
        } else {
            res = await lingkupKelasAdd({
                token: authToken,
                namaLingkupKelas: formData.nama,
            });
        }
    ...
    const handleDelete = async (row) => {
        ...
        const res = await lingkupKelasDelete({
            token: authToken,
            idLingkupKelas: row.id,
        });
    ...
    const handleAIResponse = async (result) => {
        ...
        for (const item of result.items) {
            const payload = {
                token: authToken,
                namaLingkupKelas: item.namaLingkupKelas,
            };

            try {
                const res = await lingkupKelasAdd(payload);
                const body = await res.json();

                if (!res.ok) {
                    await alertFailed(`❌ Gagal menambahkan: ${item.namaLingkupKelas}`);
                }
            } catch (err) {
                console.error(err);
            }
    ...
    const columns = [
        { key: "id", label: "ID Lingkup Kelas" },
        { key: "nama", label: "Nama Lingkup Kelas" },
    ];
    ...
    return (
        <>
            {isAdmin && (
                <ChatAssistant
                    fields={["namaLingkupKelas"]}
                    onAIResponse={handleAIResponse}
                />
            )}

            <DataTable
                title="Daftar Lingkup Kelas"
                data={data}
                columns={columns}
                onAdd={isAdmin ? handleAdd : undefined}
                onEdit={isAdmin ? handleEdit : undefined}
                onDelete={isAdmin ? handleDelete : undefined}
                searchPlaceholder="Cari lingkup kelas..."
                pagination={{
                    page,
                    size,
                    total: totalItems,
